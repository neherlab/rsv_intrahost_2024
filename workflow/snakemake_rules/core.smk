...
rule filter_lab:
    ...
    params:
        ...
        min_coverage=lambda w: f'{w.build_name}_coverage>.05', #lambda w: f'{w.build_name}_coverage>{config["filter"]["min_coverage"].get(w.build_name,10000)}',
        min_length=0, #lambda w: config["filter"]["min_length"].get(w.build_name, 10000),
